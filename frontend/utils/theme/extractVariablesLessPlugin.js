...
class PostProcessor {
    ...
    process(css) {
        ...
        const variables = selectorContents
            .split(";")
...
class Visitor {
    ...
    run(root) {
        ...
        const rules = Object.keys(variables).map(
            (variable) => `${variable.slice(1)}: ${variable};`
        );
...
class ExtractVariablesPlugin {
    constructor(options) {
        ...
        if (!options.callback || typeof options.callback !== "function") {
            throw new Error(
                "Must supply a callback function that receives the parsed variables."
            );
        }
